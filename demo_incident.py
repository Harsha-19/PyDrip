...
def run_demo():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        base_nginx_hash = calculate_sha256(nginx_path)
        base_sudo_hash = calculate_sha256(sudoers_path)
        
        print(f" Baseline Established: {os.path.basename(nginx_path)}")
        print(f"   SHA-256: {base_nginx_hash}")
        print(f"   Status : UNCHANGED")
        print(f" Baseline Established: {os.path.basename(sudoers_path)}")
        print(f"   SHA-256: {base_sudo_hash}")
        print(f"   Status : UNCHANGED")
        ...
        chk_nginx_hash = calculate_sha256(nginx_path)
        chk_sudo_hash = calculate_sha256(sudoers_path)
        
        print(f" Integrity Check [{os.path.basename(nginx_path)}]: {'MODIFIED' if chk_nginx_hash != base_nginx_hash else 'UNCHANGED'}")
        print(f"   Baseline: {base_nginx_hash[:16]}... | Current: {chk_nginx_hash[:16]}...")
        print(f" Integrity Check [{os.path.basename(sudoers_path)}]: {'MODIFIED' if chk_sudo_hash != base_sudo_hash else 'UNCHANGED'}")
        print(f"   Baseline: {base_sudo_hash[:16]}... | Current: {chk_sudo_hash[:16]}...")
        ...
        changes = [
            ScoredChange(
                ...
                old_hash=base_nginx_hash,
                new_hash=chk_nginx_hash,
                criticality="Medium",
                evidence=[]
            ),
            ScoredChange(
                ...
                change_type="MODIFIED",
                old_hash=base_sudo_hash,
                new_hash=chk_sudo_hash,
