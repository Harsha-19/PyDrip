...
class TemplateReportGenerator(ReportGenerator):
    ...
    def generate(self, investigation_result: dict) -> FinalInvestigationReport:
        ...
        try:
            from app.security_intelligence import calculate_incident_risk
    ...
    def _build_key_findings(self, files: list[dict]) -> list[str]:
        ...
        for f in files:
            ...
            if risk:
                findings.add(f"[RISK] Stage 4 — RiskEngine: Local file heuristic risk score {risk.get('score', 0.0):.2f} ({risk.get('severity', 'LOW')}) for {path}.")
    ...
    def _build_executive_summary(self, files: list[dict], overall_severity: str, risk_score: float, findings: list[str]) -> str:
        num_files = len(files)
        summary = f"The deterministic fallback investigation identified {num_files} affected file(s) resulting in an overall incident severity of {overall_severity} (global incident risk score: {risk_score:.2f}). "
