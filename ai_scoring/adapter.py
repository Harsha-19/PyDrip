...
def enrich_scan(
    ...
    try:
        ...
        # 4.5. Execute Security Intelligence Orchestration
        try:
            from app.security_intelligence import enrich_security_events
            from ai_scoring.features import build_features
            
            # Reconstruct batch_features for attribution
            batch_ts = current_detected_at
            ...
            try:
                batch_features = build_features(change_payload, hist_for_feat, detected_at=batch_ts)
            except Exception:
                batch_features = None

            enriched_data = enrich_security_events(
                changes=scored_results,
                scan_history=scan_history,
                batch_features=batch_features
            )
            final_scored_results = enriched_data["changes"]
        except Exception as e:
            logger.warning("Security Intelligence orchestration failed for scan_id=%s: %s", scan_id, e)
        ...
        update_change_scoring_records(
            db_path=db_path,
            scan_id=scan_id,
            scored_changes=final_scored_results
        )
