...
HIST_FILE = Path("data/ai_scoring_data/historical_scans.json") if Path("data/ai_scoring_data/historical_scans.json").exists() else Path("data/historical_scans.json")
DEMO_FILE = Path("data/ai_scoring_data/demo_changes.json") if Path("data/ai_scoring_data/demo_changes.json").exists() else Path("data/demo_changes.json")
...
def test_severity_fail_safes_and_compounding(dataset):
    ...
    assert sorted(res_del[0]["evidence"].keys()) == ["anomaly_reason", "criticality_weight", "drift_reason"]
    ...
    assert sorted(scored_bin[0]["evidence"].keys()) == ["anomaly_reason", "criticality_weight", "drift_reason"]

    # Compounding risk in suspicious_bulk
    scored_bulk = score_changes(demos["suspicious_bulk"], history)
    for item in scored_bulk:
        assert item["severity"] == "CRITICAL"
        assert sorted(item["evidence"].keys()) == ["anomaly_reason", "criticality_weight", "drift_reason"]
...
def test_required_vs_optional_field_boundaries():
    ...
    assert set(enriched["evidence"].keys()) == {
        "anomaly_reason", "drift_reason", "criticality_weight"
    }
    assert sorted(enriched["evidence"].keys()) == [
        "anomaly_reason", "criticality_weight", "drift_reason"
    ]
