...
_harsha_root = Path(__file__).resolve().parent.parent.parent / 'PyDrip-1' / 'PyDrip'
if str(_harsha_root) not in sys.path:
    sys.path.insert(0, str(_harsha_root))
"""
...
def test_update_change_scoring_records_updates_in_place(test_workspace):
    ...
    scored = [{
        ...
        "evidence": {
            ...
            "criticality_reason": "High criticality file.",
            "severity_reason": "Compounding severity elevation."
        }
    }]
    ...
    assert evidence_dict["criticality_reason"] == "High criticality file."
    assert evidence_dict["severity_reason"] == "Compounding severity elevation."
...
def test_enrich_scan_end_to_end_success(test_workspace):
    ...
    assert "criticality_reason" in evidence
    assert "severity_reason" in evidence
