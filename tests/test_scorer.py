...
def test_exact_added_keys():
    ...
    required_ev_keys = {"anomaly_reason", "drift_reason", "criticality_weight"}
    assert set(ev.keys()) == required_ev_keys
    assert sorted(ev.keys()) == ["anomaly_reason", "criticality_weight", "drift_reason"]
...
def test_demo_scenarios_regression():
    ...
    for scenario_name, expected_sev in expected_severities.items():
        ...
        for item in scored:
            ...
            assert sorted(item["evidence"].keys()) == [
